    ...
    for col_density in col_density_vals:
        for v in v_vals:
            for omeg in omeg_vals:
                modstr="xi%s_nH%s_v%s_omeg%s" % (str(xi),str(col_density),str(v),str(omeg))
...
p3=['v',1,1000,0.01,min(v_vals),min(v_vals),max(v_vals),max(v_vals),len(v_vals),v_array]
p4=['omeg',0,1,0.01,min(omeg_vals),min(omeg_vals),max(omeg_vals),max(omeg_vals),len(omeg_vals),omeg_array]
