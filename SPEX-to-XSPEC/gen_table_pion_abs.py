...
grid_dir='pion_absorption_grid_'+ID ##the directory containing generated spectra by PION
...
column_vals=np.logspace(-5,1,13)
    ...
    for column in column_vals:
        for v in v_vals:
            for fcov in fcov_vals:
                modstr="xi%s_nH%s_v%s_fc%s" % (str(xi), str(column), str(v), str(fcov))
...
prihd =prihdu.header
prihd.extend([('MODLNAME','PION_ABS'),('MODLUNIT','PHOTONS/CM2/S'),\
...
p2=['nh',1,np.median(column_vals),0.01,min(column_vals),min(column_vals),max(column_vals),max(column_vals),len(column_vals),col_array]
