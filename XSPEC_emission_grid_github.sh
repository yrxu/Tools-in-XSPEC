...
model_include_plasma_model="constant*tbabs*zashift*(nthcomp+relxillCp+atable{pion_xs.fits})"

type=PION ##perform absorption model scan

index_NH=25
index_logxi=24
index_v=27
index_zv=28
index_gamma_continuum=18 ###index of the spectral slope of continuum
index_gamma=26
index_norm=29
N_plasma=6 #the order of plasma model in XSPEC model combination, here is the 6th component
...
zv_min=-65000           # 0 normally 0 km/s (gas at rest)
zv_max=75000      # 105000 to have up to at least >=0.3c (after Relativistic correction)
...
echo "/*"                                                                      >> ${detection_routine}
echo "fre ${index_norm}"                                                       >> ${detection_routine}
echo "new ${index_gamma} =${index_gamma_continuum}"                            >> ${detection_routine}
...
echo "File ${input_file} does not exists"
	...
	NH=`grep "#  ${index_NH}    ${N_plasma}   ${type}TABLE  nH" ${input_file} | awk '{print $6}'`
