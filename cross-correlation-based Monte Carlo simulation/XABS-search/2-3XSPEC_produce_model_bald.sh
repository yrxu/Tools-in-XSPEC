...
echo "model zashift*mtable{xabs_xs_bald.fits}*pow"     							  >> ${routine_sim}
echo "/*"                   							  			  >> ${routine_sim}
...
do
	...
	for j in $(seq ${zv_min} ${vstep_list[$a]} ${zv_max})
		...
		echo "setplot command wd model_lw${linewidth[$a]}_xi${xi}_zv${j}_bald.qdp"     	          >> ${routine_sim}
		echo "plot "                      							  >> ${routine_sim}
		echo "setplot list"               							  >> ${routine_sim}
		echo "mv model_lw${linewidth[$a]}_xi${xi}_zv${j}_bald.qdp ${model_dir} "                  >> ${routine_sim}
